...
app.get("/filter", (req, res) =>{
  const type = req.query.type;  //type is used because of the documentation and it is also a param (/?type=example)
  const findJokes = jokes.filter(joke =>{
...
app.put("/jokes/:id",(req, res) =>{
  ...
  const findObjIndex = jokes.findIndex((joke)=>{
    if(joke.id === userId){
      return true;
    }
  })
  jokes[findObjIndex] = replace;    
  res.json(replace);
})
//6. PATCH a joke
app.patch("/jokes/:id", (req, res) =>{
  const userId = parseInt(req.params.id);
  const oldJoke = jokes.find(joke =>{
    if(joke.id === userId){
      return true;
    }
  })
  ...
  const jokeIndex = jokes.findIndex(joke =>{
    if(joke.id === userId){
      return true;
    }
  })
  jokes[jokeIndex] = replace;
  res.json(replace);
})
...
app.delete("/jokes/:id",(req, res) =>{
  ...
  if(userId >= jokes.length){
    const err = {error : "Jokes with id: " + userId + " not found. No jokes were deleted"}
    res.json(err);
  }
  const jokeIndex = jokes.findIndex(joke =>{
    if(joke.id === userId){
...
app.delete("/all", (req, res) =>{
  ...
  if(userKey === masterKey){
    ...
    const err = {err : "api Key is invalid"}
    res.json(err);
  }
})
